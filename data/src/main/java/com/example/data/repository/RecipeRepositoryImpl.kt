...
class RecipeRepositoryImpl : RecipeRepository { // todo убрать дублирование кода
    ...
    override suspend fun getRecipeById(id: String): Recipe {
        ...
        return Recipe(recipe.name,recipe.description,recipe.categoryRecipe,
            recipe.productList.map { product -> Product(product.name,product.productCategory,product.isImportant,product.count,product.maxCount) })
    }
    ...
    override suspend fun createRecipe(recipe: Recipe): Recipe {
        val mapRecipe = RecipeRequest(recipe.name,recipe.description,recipe.categoryRecipe,
            recipe.productList.map { product -> ProductRequest(product.name,product.productCategory,product.isImportant,product.count,product.maxCount) })
        val result = recipeApi.createRecipe(mapRecipe)
        return Recipe(result.name,result.description,result.categoryRecipe,
            result.productList.map { product -> Product(product.name,product.productCategory,product.isImportant,product.count,product.maxCount) })
    }
}